...
- `/posts/new`: 로그인 사용자 게시글 작성 폼입니다. `PostCreateRequest`를 받아 Service에서 작성자를 조회하고 `PostRepository.save`로 저장한 뒤 생성된 `/posts/{id}`로 이동합니다. 보드/제목/본문/태그/위치/Instagram 또는 외부 미디어 URL/`thumbnailUrl`/일정 필드를 받으며 파일 업로드와 embed는 제공하지 않습니다. HYPE 관리자 승인 행사는 ADMIN 작성자에게만 저장됩니다.
