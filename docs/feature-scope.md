...
- 보드, 제목, 본문, 태그, 위치, 이미지/영상 파일, Instagram 게시물 링크, 일정 입력 저장
- admin 전용 HYPE 관리자 승인 행사 체크 저장
- Cloudinary Free 환경변수 기반 이미지/영상 업로드
- Neon PostgreSQL 배포 설정 문서화
- Live Preview에서 첨부 이미지/영상 원본 비율 미리보기
...
- MVP에서는 Cloudinary Free를 외부 스토리지로 사용해 이미지/영상 파일 첨부를 지원합니다.
- 게시글은 첨부 파일 또는 인스타그램 게시물 링크를 받을 수 있습니다.
...
- 작성 폼은 이미지/영상 파일 입력과 Instagram 게시물 링크 입력을 제공하며, `thumbnailUrl` 직접 입력은 사용자 화면에서 노출하지 않습니다.
- SHOW 외 보드는 `eventDate`/`deadline` 입력 영역을 제공하고, HYPE 관리자 승인 행사 체크는 admin에게만 노출/저장됩니다.
- Live Preview는 입력값과 선택한 로컬 파일 미리보기를 보여주며 실제 embed는 하지 않습니다. 첨부 이미지/영상은 원본 비율을 유지하고, Instagram 링크 카드는 Instagram URL이 있을 때만 별도 붉은 카드로 보여줍니다.
- 목록은 업로드 결과의 `thumbnailUrl`을 카드 이미지로 사용하고, 상세는 이미지/영상 자체와 Instagram 링크 카드를 분리해 표시합니다.
...
- 공개 작성자 프로필의 Comments와 Activity는 숨김 게시글 또는 차단 작성자 게시글에 달린 기록을 노출하지 않습니다.
...
- 마이페이지 편집은 URL 기반 프로필/헤더 이미지 갱신만 제공하며 새 이미지 원본 추가, 직접 이미지 업로드, 직접 영상 업로드는 제외합니다. 차단된 기존 세션은 프로필/계정/포트폴리오/참여 행사 수정 액션을 수행할 수 없습니다.
- Instagram 링크가 상세/프로필 영역에 노출될 경우 새 탭으로 열며, 첨부 파일은 Cloudinary 업로드 URL을 사용합니다.
