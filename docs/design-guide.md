...
- 장르 필터는 보드 탭과 어울리는 붉은 언더라인 탭 형태이며, `All`은 필터 초기화, 나머지 영문 장르는 다중 선택 상태를 시각적으로 표시합니다.
- 선택 장르가 많아질 수 있으므로 모바일에서는 장르 버튼 줄을 가로 스크롤로 유지해 버튼 텍스트가 찌그러지지 않게 합니다.
- 카드 하단에는 `Instagram` 새 탭 링크와 기존 `/dancers/{id}` 기반 `visit profile` 링크를 함께 둡니다.
...
- 헤더 `Add New`에서 진입하며 로그인 사용자만 접근합니다.
- 레이아웃은 좌측 작성 폼, 우측 Live Preview 2컬럼입니다.
- 좌측은 `CREATE POST`, `TELL US WHAT MOVES YOU.` 큰 타이포, 보드 언더라인 탭, 선 중심 입력 필드, 넓은 `PUBLISH` 버튼으로 구성합니다.
...
- 우측 Live Preview는 밝은 회색 배경에 실제 상세/카드 톤과 연결되는 게시글 미리보기로 구성합니다.
- Preview는 보드 배지, 제목, 작성자, URL 기반 썸네일 또는 placeholder 미디어 영역, 본문, 위치/일정, 태그를 실시간 반영합니다.
- 실제 Instagram embed나 외부 미디어 iframe은 사용하지 않습니다.
- 다크 모드에서는 같은 구조를 유지하되 우측 패널과 입력 라인의 대비를 보강합니다.
