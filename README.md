...
- URL 기반 썸네일 입력과 작성 중 Live Preview
- 라이트/다크 모드
- Render 무료 티어 배포 대응

현재 MVP는 직접 영상 업로드, 직접 이미지 업로드, 외부 미디어 embed를 제외합니다. 게시글/미디어 데이터는 로컬 시드와 URL 기반 필드로 관리하며, 작성 폼에서도 Instagram/외부 미디어 URL과 `thumbnailUrl`만 입력합니다.
